...
static int test12(void){
    ...
    int arg = 0;
    for(int i = 0; i < 128; i++) {
        if (pthread_create(&tid1, NULL,  &_arg_crap, &arg) == -1)
        {
            usleep(10000);
            i--;
        }
...
static int (*test_arr[NUM_TESTS])(void) = {&test0, &test1, &test2, &test3, &test4, &test5, &test6, &test7, &test8, &test9, &test10, &test11, &test12, &test13};
